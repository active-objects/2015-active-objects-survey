...
A first synchronization point in the example is reached at
...
synchronization leading to deadlocks.  Deadlocks can also be removed
...
(e.g., Java) each thread is a physical one. In that case, one can
...
object-oriented model.  Annotations allow to define custom schedulers
...
along with the start of this thread executing the method. Here is a
...
at runtime.  The only drawback is that if a method call contains a
recursive stack of synchronous calls, this stack needs to be saved
upon preemption. This scenario cannot be realized with lambda
expressions.
...
local references, acting like standard references types.
...
code generated with the legacy Java backend of ABS run on a single
machine, and native code written in ProActive.
%
...
time. In ProActive we instantiate two workers on each machine (each
machine has two dual-core CPUs).
...
machines and then decrease at a slower rate. The code generated with
the legacy Java backend has an optimal degree of parallelism of 4
workers (i.e., the number of cores of the machine); execution time
even increases afterwards due to context switching.  In contrast to
