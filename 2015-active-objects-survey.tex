...
\begin{abstract} % 92 words/100
  ...
  synchronisation and communication between parallel entities. Among
  them, the actor model is based on loosely coupled parallel entities
  that communicate trough asynchronous messages thanks to the use of
...
organising the program in terms of well-defined entities called actors. Each
...
paradigm. Such languages are often called \emph{active object}
languages. Compared to the basic unit of concurrency, which is the
same in all actor languages, the way methods are called seems to pose
a rather subtle distinction. Although this causes merely minor
...
abstractions given to the programmer. By supporting
\emph{communication by asynchronous method calls}, active object
languages provide a programming model that is tightly integrated with
the object-oriented programming paradigm and its programming
...
their main focus, having been conceived as modelling languages: actors do
...
runtime support for the presented languages and
...
returning values from a method call can be modelled by another
...
point. This kind of explicit synchronisation can make concurrent
...
As usability has been a primary goal in designing Rebeca, the conrete
...
can be modelled with Probabilistic Timed Rebeca
...
\ref{line:rebec:send:self}). Since communication is asynchronous,
...
(line \ref{line:rebec:switchStart}).  Finally, the \texttt{main} block
...
variables allow some synchronisation between actors reminiscent of
...
Formal semantics of Probabilistic Timed Rebeca is defined interms of
...
standard imperative layer. So far, this is very much like Scala-light.
...
principally possible \cite{abraham05tcs,BlomHuisman14}, is highly
complex and seems to be out of scope at the moment for relaxed memory
consistency models. To achieve feasibility, the concurrency model of
ABS carefully restricts the possible interactions between concurrent
tasks, which stem from method activations, while still allowing to
describe complex, realistic behavior of asynchronous systems.  More
precisely, the ABS concurrency model is based on cooperative
...
Section~\ref{sec:implem-java-8} illustrates a possible implementation
of cooperative scheduling for ABS.
...
We explain the concurrency model of ABS with help from the code in
...
\lstinline{Server}.  The directive \lstinline{local} places the
...
might well be aliased. The standard ABS idiom for asynchronous method calls
...
performs blocking synchronisation when needed. ProActive
...
wait-by-necessity synchronisation is automatically triggered upon an access to an unresolved future. 
...
\code{v.foobar()}, wait-by-necessity synchronisation automatically
...
\end{lstlisting}
In this example, two groups of requests are defined, each with one method.  The two groups are declared to be compatible (and
so are their methods, by extension). The \code{selfCompatible}
parameter defines whether two different requests of the same group are
allowed to run in parallel. At runtime, a ready request is
automatically executed if it is compatible (i) with requests that are
already executing and (ii) with older requests in the queue. The first
...
with currently executing requests are continously overtaking requests
...
further requests as compatible and prevent undesired behaviour with
traditional low-level Java synchronisation primitives. Such a mixed
...
involved in wait-by-necessity synchronisation.  Additionally, threads
...
\paragraph{Degree of synchronisation}
In ASP, the only blocking synchronisation is wait-by-necessity on a
...
annotation is specified.  However, synchronisation only occurs when
...
requiring any additional synchronisation, which limits blocking
synchronisation leading to deadlocks.  Deadlocks can also be removed
...
application is localised in few files and does not require application
...
on one hand verify the correct behaviour of the application using the
...
on active objects developed since 2014 in the \Upscale
project.\footnote{EU project FP7-612985 \Upscale: From Inherent
  Concurrency to Massive Parallelism through
  Type-based~Optimisations.}  The language has been designed to excel
...
locality, \textit{type-based synchronisation directives} provided by a
...
\paragraph{Degree of Synchronisation}
In Encore, the synchronisation constructs \ec{get} and \ec{await}
...
%
Furthermore, Encore provides the \emph{future chaining} operator ($\rightsquigarrow$),
which registers a callback (as an anonymous function, aka lambda) to the future and
...
In Encore, regular
% non-future variables 
and future variables are distinguished statically (e.g., \ec{int} vs.\
\ec{Fut int}).  The \ec{get} operation explicitly extracts the content
from the future type.
%\ec{get} converts a future
%type to non-future type  by extracting the content explicitly.
Both in future management and with parallel combinators, the programmer is exposed to the concurrent computations
...
Pony language (PonyRT), which provides a fully concurrent garbage
collector~\cite{DBLP:conf/oopsla/ClebschD13} and actor library among
other features.  The Encore runtime (EncoreRT), extends the PonyRT
with new features such as futures, the \parT{} abstraction, a task
library and the notion of \emph{encore threads} (explained
below). These features are part of the runtime and therefore, written
in C.  On top of these libraries rests the Encore Standard Library,
which is written in Encore itself.
...
\begin{figure}[t]
	...
	\caption{(Left) An Encore program executing a \ec{get} on an
		...
		the future has been resolved. Once the object gets to the
		...
		executing an \ec{await} on an unresolved future. In
...
aside and a new stack is given to the Encore thread, so that it can
continue running some other or the same active object using the new
stack.  To make this process more efficient, Encore has a stack pool
that allows reusing unused stacks.  Upon fulfilment of the awaited
future, the Encore thread can continue processing the paused execution
and, after finishing, the stack is collected and returned to the stack
