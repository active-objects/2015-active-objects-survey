...
serves requests in a COG multi-active object: requests do not run in
parallel but interleave upon wait-by-necessity (simulation of \lstinline{await}). In a similar manner we
can simulate the blocking of \lstinline{get} by a suitable (hard)
limit. The implementation details can be found in~\cite{rochas:hal-report}.
We have formally proven that all ABS constructs are correctly translated by the ProActive 
backend, i.e., the ASP translation simulates all  rules of ABS' operational
semantics, and  all translated ASP configurations correspond to a valid ABS 
execution. 
The conclusion of this work is that we could correctly simulate constructs of ABS that do not exist in ProActive. The proof of correctness highlights the differences of the two languages, especially regarding futures. In ProActive the synchronisation is not done upon request termination like in ABS. Somehow, we compared the data flow synchronisation of ProActive (where futures are transparently updated) to the control flow synchronisation of ABS. In the ProActive translation of an ABS program, a future might be updated before than in the ABS case, because an explicit future read not been performed yet. This is an intrinsic difference of the two languages, that cannot be exactly simulated.
...
For evaluation purposes we use a representative HPC application,
...
than the experiment reported in Section~\ref{sec:implem-java-8}. This
is adequate, because ProActive active objects rely on physical threads
...
backend for ABS run in distributed mode, the code generated with the
legacy Java backend of ABS run on a single machine, and native code
written in ProActive to illustrate the performance of the ProActive
library.
%
Map instances (workers) are created in their own COG to make them work
in parallel.  We consider a searched pattern of 250 bytes, and a
database of 5 MB of DNA sequences.  Each map searches for the maximum
matching sequence of a chunk.  Then, a reducer outputs the global
...
backends application from 2 to 50 workers, therefore using from 1 to
