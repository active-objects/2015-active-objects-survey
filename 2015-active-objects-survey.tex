...
A major  challenge in the development of high-level
...
based on loosely coupled processes which interact via  asynchronous message
...
in  \cite{Hewitt:1973} the Actor model of computation originated from the need \emph{to
efficiently run the coming generation of PLANNER-like  artificial intelligence languages including those requiring a  high degree of parallelism} (quote from \cite{Hewitt:1973}).
 Erlang~\cite{Erlang-book} is
...
In the above mentioned original Actor models of computation the interpretation and processing of the messages are confined to and part of the overall internal behavior of an acor. This however does not allow for a clear separation of concerns between communication and computation, and as such does not support a ``programming to interfaces'' discipline.
A major
...
construction of programs in terms of modules and their interfaces.  Modules are used to
...
Active (or concurrent) object languages integrate the basic Actor model with
object-oriented concepts and as such do support interface abstractions of implementation details.   This integration was first proposed in 
...
As in the actor-based language Rebeca,
...
return values, while maintaining a loose coupling between actors,
leads to the notion of future variables.  Futures were originally
discovered by Baker and Hewitt in the 1970s~\cite{BakerHH77}, later
rediscovered by Liskov and Shrira as promises in
Argus~\cite{Liskov88}, by Halstead in
...
in stable, public distributions. Those four language also have a formally defined 
semantics allowing us to precisely compare the behaviour of the programs written in the 
different languages.
...
A discussion of other hight-level concurrent languages can be found in Section~\ref{sec-discussion}. 
...
Another synchronisation constraint that exists  is related to message 
...
Data sharing occurs between threads and is related to the question of which objects 
are active.
...
Table~\ref{tab::Rebecacomparison}).  

%
Extended Rebeca~\cite{DBLP:conf/acsd/SirjaniBMS05,DBLP:journals/jucs/SirjaniBM05} promotes the idea of actors being grouped in components, where we have globally asynchronous communication among components, and within a component we may have locally synchronous communication.
%
RebecaSys is developed to support hardware/software co-design (or system-level design) \cite{DBLP:journals/tecs/RazaviBSKSS10}. In RebecaSys the full asynchrony of Rebeca is jeopardise by adding the wait statement in order to be faithful to the target design language, SystemC. Global variables are added, but can only be used in a controlled manner.
%
In Variable Rebeca, the modeller can use annotation to model variable behaviours and hence define a product line of actors with different computations \cite{DBLP:journals/jucs/SabouriK13}. In Variable Rebeca the semantics of Rebeca is fully preserved. All the other members of Rebeca family can be extended by the annotation mechanism that is used in Variable Rebeca.
%
In Broadcasting Rebeca \cite{DBLP:conf/fsen/YousefiGK15} and Wireless Rebeca \cite{DBLP:journals/corr/YousefiGK16} the focus are on modelling and verifying of different network protocols, and hence they provide alternative message passing mechanisms, like broadcasting  and multi-casting among actors, respectively.


Timed Rebeca \cite{DBLP:journals/corr/abs-1108-0228,DBLP:journals/scp/ReynissonSACJIS14} extends Rebeca by adding a few keywords to the syntax of Rebeca in order to model real-time behaviour and is supported by customised efficient analysis
tools. Probabilistic Timed Rebeca \cite{DBLP:journals/eceasst/JafariKSH14} extends Timed Rebeca to capture probabilistic behavior and as for now can only be analyzed using back-end tools.
%
Extending Rebeca with time and probability is orthogonal to other extensions, and where the core semantics of Rebeca is preserved these extensions can be added on top of other extensions. More specifically, Variable Rebeca, and Broadcasting and Wireless Rebeca can be extended by time and probability in the same way as in Probabilistic Timed Rebeca.
...
\emph{actors} (also called \emph{rebecs} in Rebeca). A reactive class specifies the size of its message queue (Line 1) and may declare \emph{state variables} (Line \ref{line:rebec:statevar}).
...
conditionals (Lines
\ref{line:rebec:switchStart}--\ref{line:rebec:switchEnd}), loop statements, and message
sending statements (Lines \ref{line:rebec:send:receiveMovie},
\ref{line:rebec:send:reqForMovie}). In Rebeca, periodic behavior of a system is
...
\paragraph{Degree of data sharing} Rebeca is a typical \emph{uniform active object} 
language and 
as such guarantees at the same time absence of data sharing and of  copy of data.
...
e.g., in Fig.~\ref{fig:rebeca-example} Line \ref{line:rebec:reqForMovieEnd} \texttt{Dispatcher} sends a request to the server \texttt{m3} and passes the reference to the client to \texttt{m3} as a parameter instead of passing a fresh copy of the client actor.
...
computations are, therefore, deterministic. 
Communication is asynchronous in ABS, and no particular ordering has to be ensured on 
request communication and service.
Obviously, execution in
...
\lstinline{await}. Using these features, the user is exposed to the notion
of asynchronous vs.\ synchronous calls, futures, and thread
interleaving, but the language still retains a compositional proof theory
\cite{SDE:BoerCJ07}.

\paragraph{Degree of data sharing} There is no designated active object in a COG, all 
objects may be
...
method calls.
ABS features a \emph{concurrent object-group}  model where objects in the same group can 
safely be invoked directly because they are manipulated by a single thread, invocation 
between different COGs is by nature asynchronous.
...
Encore Standard Library, which is written in Encore itself.\TODO{add a ref to encore 
public repository}
...
Akka~\cite{haller2009scala,AkkaBook}  is a
scalable library for implementing actors on top of Java and Scala. 
Akka Typed Actors is an implementation of the Active Objects pattern. 
%
...
%frameworks 
%Akka and Orleans, from the point of view of the languages presented here. The success of 
...
%
%\paragraph{Akka}
%Akka~\cite{haller2009scala,AkkaBook} is an actor-based message-driven runtime for managing concurrency, elasticity and resilience on the Java Virtual Machine with support for both Java and Scala. 
%It is an open core development platform for Akka distributed systems runtime, play web framework, lagom microservices framework, and Apache Spark in-memory compute engine for Java and Scala. 
All interactions of actors in Akka use purely message passing and everything is asynchronous. 
%Programmers need to provide a pattern match for all messages that it can accept. 
%To be able to handle unknown messages, a default case is required. 
The way actors interact is the same whether they are on the same host or separate hosts, communicating directly or through routing facilities, running on a few threads or many threads, etc. Such details may be altered at deployment time through a configuration mechanism, allowing a program to make use of more (powerful) servers without modification. 
Akka is ideally suited for hybrid cloud architectures and the elastic scaling of cloud platforms. In an experiment with Google, it only took 4 minutes to start a 1000 node Akka cluster on Google Compute Engine (GCE), which included the time to start the GCE instances. 
%In Akka, actors form a tree with actors being parents to the actors they've created. As a parent, the actor is responsible for handling its children's failures (supervision), forming a chain of responsibility. 
%Accordingly, 
Besides, Akka supports error handling. 
It embraces the reality of unplanned errors and adopts a pragmatic ``Let It Crash'' philosophy using supervision and self-healing to ensure impacted components are reset to a stable state and restarted upon failure.

%\paragraph{Orleans}
Orleans~\cite{BykovGKLPT11,BernsteinB16} is an actor framework developed by Microsoft 
research and used in several 
Microsoft products including online games relying on a Cloud infrastructure. The 
...
the model is called a ``\emph{virtual actor model}''.
Orleans 
...
single application. The Orleans runtime also supports the application in the management 
