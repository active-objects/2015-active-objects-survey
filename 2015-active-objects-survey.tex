...
control communicate and synchronise directly via shared data.  For
...
like point-to-point FIFO, adds synchronisation which can lead to a
...
order by explicit synchronisation is more costly and more constraining
...
the same time analysable modeling language.  It has a formal semantics
...
value. Their task is to initialise the actor's state variables (Line
...
tightly coupled component to Rebeca; actors inside a component can be
...
\item Model checking tool for LTL and CTL model checking of Rebeca, as
  well as Floating Time Transition System generator and analyser for
  Timed Rebeca
\item State space generators for
% compatible with 
  CADP and $\mu$CRL analysis and visualisation tool sets
...
\cite{johnsen15jlamp} which have been used to, e.g., model and analyse
...
\node[rectangle,
          ...
          {History-Based Behavioural Interface Specification};
...
Boolean argument and can be used to synchronise with another task. In
...
\lstinline{init()} has not been already scheduled, it will be
...
important point is that no data races can occur between explicit
synchronization points (\lstinline{release}, \lstinline{await}), which
makes computations deterministic.  Communication is asynchronous in
ABS, and no particular ordering has to be ensured on request
communication and service.  Obviously, execution in general is
...
still retains a compositional proof theory \cite{SDE:BoerCJ07}.
...
is available \cite{BFH14,WongAMPSS12}.
...
collaboratory \cite{domenech17fase} that runs in a browser and permits
to try out most ABS tools. The collaboratory can be used either as a
service or installed locally. Here is a list of currently supported
tools for ABS:
...
\item Visualisation of ABS model execution as a sequence diagram
...
\item A glass box test case generator for \emph{concurrent} models
  \cite{AlbertAGM15}
\item A sound deadlock analysis tool \cite{CGM:SoSym2014}
...
\end{lstlisting}
This example defines two groups of requests, with one
method each.  The two groups are declared to be compatible (and so are
...
ordering of incompatible requests and prevents starvation, which would
arise when requests of a group $G$ that are merely compatible with
currently executing requests are continuously overtaking requests of
another group $G'$ with which they are incompatible.
...
parallelism.  If even more parallelism is required, beyond what is possible with
request compatibility, then the programmer can define
...
for reentrant calls, especially if no compatibility annotation is
...
transmission~\cite{HKRZ:Coregrid:2010} (a ProActive future is
serializable).  Deadlocks can also be removed by using multi-active
...
\paragraph{Degree of transparency}
In ASP the programmer is not exposed to the notion of a future and in
...
they are copied together with all their dependents to the destination
...
Coherency between different copies of a passive object is not
...
Caromel et al.~\citeyear{CHS:POPL04} formalise the mono-threaded part
...
Whenever an active object or a future is given as a call parameter to an
...
objects on distributed infrastructures, ProActive has a
deployment specification mechanism which makes the physical
deployment independent from the deployment logic. This is realized by
 binding virtual node names, used in the source code,
 to machine addresses or names. In practice, this binding is
implemented in XML configuration files. Since the binding is happens at
\emph{deployment time}, changing infrastructure for a ProActive
application is localized in few files and does not require 
...
options (e.g., fault tolerance or logging).
...
language based on active objects developed since 2014 in the EC funded
\Upscale project.
...
Cholesky factorisation in parallel, spawning tasks (Lines
...
(\ec{bind :: Par[t] -> (t -> Par[t']) ->} \ec{Par[t']}),
%
...
class Mtx
  ...
  def computeFastestDiagonal(): Par[Mtx]
    ...
    %\label{line:encore:pipe}%getDiagonalMtx <<  bind(mtvInv, par)
  end
end
\end{lstlisting}
...
We have here highlighted the most important combinators, others are
...
Encore provides a \emph{future chaining} operator
...
AmbientTalk~\cite{Dedecker:2006:APA:2171327.2171349}, but it
...
Communication in Encore is asynchronous but requests are served in a
FIFO order.
...
the on-going concurrent computations. However, especially with future chaining and
parallel combinators, the scheduling and ordering of operations is
automatic and the programmer only expresses concurrency from a
...
The fact that an implementation has support for distribution or not
...
ABS allows several programming paradigms and design patterns offering
both a functional and an object-oriented model.  Annotations support
custom schedulers to be defined to ease the development of batch systems
and workflows. The main goal of the Java~8 backend\footnote{Available
  at
...
latencies and performance issues which are not easy to
observe at a modeling level. The version of the Java 8 backend for
ABS presented below does not support distributed execution.
...
New features of Java 8 allow method calls to be wrapped in
lightweight lambda expressions which can be put on the
...
Figure~\ref{abs8:coop} shows how active objects in this implementation
...
of this approach will be evaluated on an ABS example comparing the
...
drawback is that if a method call contains a recursive stack of
synchronous calls, this stack needs to be saved when encountering an
\texttt{await} statement, which cannot be realized with lambda
expressions. To solve this problem, such a call stack can be modeled
by continuation functions and then saved as lambda expressions. These
lambda expressions are ordered such that, once released, the
continuations can execute and maintain the correct logic of the
program.
...
remote method invocation package to implement the communication layer
between active objects, although other communication protocols are
possible.  This fact accounts for most of the implementation specifics
mentioned below.
...
Garbage collection in ProActive is deeply linked with how objects are
...
developed in ProActive which detects useless
active objects (those that are idle and only referenced by idle active
objects). This is handled by a form of common agreement based on the
reference graph between active objects~\cite{CCH:Middleware07}.
...
models in distributed environments, with less optimisation than
...
some data. In ABS synchronization on futures is \emph{control-flow
...
be safe as well.
%
The integration such shared passive objects with capability types is
...
scheduled by the default, reduction-based Erlang scheduler of Erlang.
...
\newline $\bullet$ Multiactive  objects\newline
                                                                    with
                                                                    parallel
                                                                    combinators
                                                                    for
...
\caption{\label{fig:review}The surveyed active object
  languages according to different design dimensions.}
\end{figure}


\paragraph{Degree of synchronisation}
The degree of synchronisation is a design choice that is central to
...
strict synchronisation exists. This resembles the principles of
...
perspective, synchronisation hinders efficiency and parallelism, but
makes programming easier, and sometimes more efficient. The other
programming languages we presented allow programs to synchronise on
the result of a method invocation through the use of futures. However,
different synchronisation patterns exist on futures: from strict
synchronisation like the \texttt{get} operator of ABS to asynchronous
reaction to future fulfillment enabled by future chaining in
Encore. Two synchronisation patterns on futures presented in this
article feature an interesting compromise: the \texttt{await}
operation of ABS and Encore enables \emph{cooperative multi-threading}
and let the program serve another request while the future is awaited;
and the \emph{wait-by-necessity synchronisation} of ProActive that
...
\paragraph{Degree of transparency}
The degree of transparency is very much related to the synchronisation
primitives.  In practice there is for the moment mostly two extreme
...
objects and where synchronisation on method results is hidden to the
programmer. In Rebeca, explicitness is even stronger as the remotely
invokable methods are declared by a specific syntax and do not return
a result.
...
non-uniform active object model where some active objects can
be invoked from anywhere and others are transmitted by copy. ABS was
...
It is interesting to notice that both ABS and Encore allow the
...
the languages that can be checked and the different language semantics
that can be verified. ABS on the contrary comes with very different
analyses based on the same semantics. Both languages show the
advantage of adopting an active object programming paradigm compared
to standard models of distribution and concurrency, for the
verifiability of programs. ASP and Encore mainly
focused on execution support. While powerful verification
tools also exist for those languages, significant efforts have been
put into the efficiency of local parallelism in Encore, and to
support effective large-scale distribution in ProActive. ProActive
programs target HPC applications: setting up the program environment
is complex, but the distributed execution is fairly efficient; only a
few active objects co-exist in an application. The Java 8 backend for
ABS provides light threads and enables the instantiation of many active
objects on the same machine. It also permits active objects to achieve
faster communication than in ProActive. At a finer granularity, Encore
provides optimised constructs for safe and efficient
...
to minimise implementation effort all the languages presented here
...
There are several other programming languages that introduce
high-level abstractions for concurrent programming that deviate from
the active object approach.  Java Annotations for Concurrency
(JAC)~\cite{ref:jac} supports annotations to specify the kind of
parallelism that can occur inside a Java object from a relatively
high-level perspective. It is possible to encode versions of actors in
JAC.
%
The concurrency model of
AmbientTalk~\cite{Dedecker:2006:APA:2171327.2171349} is based on the E
...
modification.  Akka is well suited for hybrid cloud architectures and
